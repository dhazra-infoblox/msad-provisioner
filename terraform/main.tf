    ...
    mainSteps = [{
      ...
      inputs = {
        runCommand = [
          ...
          "  $stale = @($sd.DACL | Where-Object { $_.Trustee.SIDString -eq $sid.Value })",
          "  if ($stale.Count -gt 0) { Write-Host \"[WMI] Replacing $($stale.Count) existing ACE(s) for $account\"; $sd.DACL = @($sd.DACL | Where-Object { $_.Trustee.SIDString -ne $sid.Value }) }",
          "  $sd.DACL += $ace",
    ...
    mainSteps = [{
      ...
      inputs = {
        runCommand = [
          ...
          "foreach ($ip in $allIps) { Enable-WSManCredSSP -Role Client -DelegateComputer $ip -Force }",
          "Write-Host \"CredSSP client enabled for: $($allIps -join ', ')\"",
          "",
          "# --- TrustedHosts ---",
          "$trustedHosts = ($allIps -join ',')",
          "Set-Item WSMan:\\localhost\\Client\\TrustedHosts -Value $trustedHosts -Force",
          "Write-Host \"TrustedHosts set to: $trustedHosts\"",
          "",
          ...
          "",
          "# Enable AllowFreshCredentials (Kerberos)",
          ...
          "$i = 1",
          "foreach ($ip in $allIps) { Set-PolicyFileEntry -Path $polPath -Key \"$regPath\\AllowFreshCredentials\" -ValueName \"$i\" -Data \"WSMAN/$ip\" -Type String; $i++ }",
          "Write-Host \"GPO Registry.pol: AllowFreshCredentials set for: $($allIps | ForEach-Object { \"WSMAN/$_\" })\"",
          "",
          ...
          "$i = 1",
          "foreach ($ip in $allIps) { Set-PolicyFileEntry -Path $polPath -Key \"$regPath\\AllowFreshCredentialsWhenNTLMOnly\" -ValueName \"$i\" -Data \"WSMAN/$ip\" -Type String; $i++ }",
          "Write-Host \"GPO Registry.pol: AllowFreshCredentialsWhenNTLMOnly set for: $($allIps | ForEach-Object { \"WSMAN/$_\" })\"",
          "",
          "# --- Apply group policy ---",
          "gpupdate /force",
